...
description: Build and sharpen a project's domain model. Use when domain terminology, relationships or rules change, when writing or editing a CONTEXT.md, or when recording or editing an ADR.
---
...
When the user uses a term that conflicts with the existing language in `CONTEXT.md`, surface the discrepancy and reconcile it against accepted decisions. If the intended meaning remains unresolved, ask: "Your glossary defines 'cancellation' as X, but you seem to mean Y. Which is it?"
